...
// ── Icônes des cartes KPI — répliques inline lucide-react (Eye / FileText / Globe / Clock),
// même patron que les icônes ci-dessus (taille configurable via `size`).
...
// ── KPI ──
export function Kpi({ label: lbl, value, narrow = false, icon, tint }: { label: string; value: string | number | null; narrow?: boolean; icon?: ReactNode; tint?: string }) {
  ...
  return (
    ...
    <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 12, padding: narrow ? 12 : 16, ...(narrow ? { flex: '1 1 calc(50% - 6px)', minWidth: 0 } : { flex: 1, minWidth: 140 }) }}>
      {icon && (
...
export function ColManager({ anchorRef, cols, labelFor, onChange, onSave, defaults, onClose }: {
  ...
  const { draggingId: dragId, overTarget: over, dragPos, startDragMouse, startDragTouch } = useDragReorder({
    cols, onChange: (next) => { onChange(next); onSave(next) },
  })
