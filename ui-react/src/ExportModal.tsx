 ...
 * Format Excel (.xlsx) / CSV, panneaux Incluses / Exclues réordonnables par
 * glisser-déposer, export de TOUTES les lignes via `fetchAll`. La brique ne peut
 * PAS importer xlsx (React/modules externalisés) : on réutilise l'instance XLSX
 * exposée par l'hôte (`window.MelisXLSX`) ; si absente, on retombe sur le CSV.
 * Styles inline + variables CSS du thème, i18n FR/EN lu depuis <html lang>.
 * ────────────────────────────────────────────────────────────────────────── */
...
export function ExportModal<T>({ cols: colsProp, labelFor, fetchAll, getCell, filename, sheetName, total, onClose }: {
  ...
  // Single source of state (like ColManager): `included`/`excluded` are derived from `.visible` on
  // every render, rather than two separate arrays — that's what `useDragReorder` expects (same hook
  // as ColManager, touch-compatible drag; the native HTML5 `draggable` API never fires from touch).
  // Its panel names stay 'visible'/'hidden' internally; the displayed labels ('Included'/'Excluded')
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,.5)', ...(narrow ? { padding: 12 } : {}) }}
      onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{ ...card, width: '100%', maxWidth: 480, ...(narrow ? { maxHeight: 'calc(100vh - 24px)', overflowY: 'auto', boxSizing: 'border-box' } : {}) }}>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Étroit : panneaux empilés — côte à côte, chacun tombe sous ~140px et les libellés
              de colonnes deviennent illisibles. */}
