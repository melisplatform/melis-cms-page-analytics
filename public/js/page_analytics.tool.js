$(function() {
  ...
  $body.on(
    "change",
    "form#select_page_analytic_form_analytics_content select#analytics-content-side-id",
    function() {
      /**
       ...
       */
      $(
        '<div class="melis-cms-page-analytics-temp-overlay"></div>'
      ).insertAfter("#id_meliscms_page_analytics_content");
  ...
  $body.on(
    "change",
    "form#select_page_analytic_form select#site_id",
    function() {
      var siteId = parseInt($body.find(this).val());

      if (!isNaN(siteId)) {
        var page_analytics_id = $body
          .find("#select_page_analytic_form")
        ...
        $.post(
          "/melis/MelisCmsPageAnalytics/MelisCmsPageAnalyticsTool/getSiteAnalytics",
          { site_id: siteId },
          function(data) {
  ...
  $body.on("change", "select#page_analytics_id", function() {
    ...
    var analyticsKey = $body.find(this).val(),
      siteId = parseInt(
        $body.find("form#select_page_analytic_form select#site_id").val()
      );

    if (!isNaN(siteId) && analyticsKey === "melis_cms_google_analytics") {
      $body
        .find("div#melis-cms-google-analytics-guidelines")
        .removeClass("hidden");
      $body
        .find("div#melis-cms-google-analytics-guidelines")
        .css("opacity", "0");
      ...
      $.ajax({
        ...
        data: {
          ...
          site_id: siteId,
          fileChanged: privateKeyFileChange,
        },
        dataType: "html",
        encode: true,
      })
        .done(function(data) {
          if (data) {
            ...
            var privateKeyFileName = $body.find(
              "#id_google_analytics_private_key_val"
            );
            if (privateKeyFileName.length) {
              privateKeyFileName = privateKeyFileName.val();
            } else {
              privateKeyFileName =
                translations.tr_melis_cms_google_analytics_private_key_non;
            }

            var disbaleFlag = $body.find("#id_google_analytics_private_key");
            if (
              disbaleFlag.length &&
              disbaleFlag.attr("disabled") === "disabled"
            ) {
              disbaleFlag = true;
              privateKeyFileName =
                translations.tr_melis_cms_google_analytics_private_key_no_rights;
            } else {
              disbaleFlag = false;
            }
            $body.find("#id_google_analytics_private_key").filestyle({
              ...
              input: true,
              buttonText:
                translations.tr_melis_cms_google_analytics_private_key_upload,
              placeholder: privateKeyFileName,
            });
            melisHelper.removeLoadingZone(
              $body.find("#analytics-settings-form")
            );
            ...
            $.post(
              "/melis/MelisCmsPageAnalytics/MelisCmsPageAnalyticsTool/getAnalyticsScript",
              {
                site_id: siteId,
                analytics_key: analyticsKey,
              },
              function(data) {
                ...
                melisHelper.removeLoadingZone(
                  $body.find("pre#pads_js_analytics")
                );
              }
            );
          } else {
  ...
  $body.on("submit", "form#select_page_analytic_form", function(e) {
    var formData = new FormData(this),
      editor = ace.edit("pads_js_analytics"),
    ...
    $.ajax({
      ...
      .done(function(data) {
        if (data.success) {
          ...
          melisHelper.zoneReload(
            "id_meliscms_page_analytics_display",
            "meliscms_page_analytics_display"
          );
        } else {
  ...
  $body.on(
    "click",
    "a.melis-cms-page-analytics-refresh-table-tool",
    function() {
      var siteId = $(
        "form#select_page_analytic_form_analytics_content select#analytics-content-side-id"
      );
      if (siteId.length) {
        ...
        if (siteId > 0) {
          melisHelper.zoneReload(
            "id_melis_cms_page_analytics_tool_table",
  ...
  $body.on("click", "a.melis-cms-page-analytics-refresh", function() {
    ...
    melisHelper.zoneReload(
      pageId + "_id_melis_cms_page_analytics_page_table",
